...
- 电源模式、Token 用量、温度监测、服务监测四个模块均可点击标题或右侧箭头折叠；折叠后只保留首行的关键数值与切换控件，面板高度随之收缩。电源模式切换器右侧用图标显示当前实际状态（Server 机架 / Sleep 月亮 / Normal 太阳 / 未知问号，切换中显示进度）。
- 服务监测中的 TCP 端口胶囊本身即为链接，点击在浏览器打开对应的本地 `http://` 地址，不再附加链接图标。
